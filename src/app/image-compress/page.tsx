...
import { compressImage } from '@/lib/ffmpeg/imageCompress';
...
export default function ImageCompressPage() {
  ...
  const handleCompress = async () => {
    ...
    try {
      setProgress(30);
      const result = await compressImage(file, quality / 100, maxSize, maxSize);
      setProgress(100);
      setResult(result);
      setStatus('complete');
    } catch (err) {
    ...
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-3 gap-4 text-center">
        ...
        <div className="p-4 bg-green-50 rounded-lg">
          ...
          <div className="text-xs text-gray-500">批量处理支持</div>
        </div>
        <div className="p-4 bg-purple-50 rounded-lg">
          <div className="text-2xl mb-2">🎯</div>
          <div className="text-sm font-medium">可调质量</div>
          <div className="text-xs text-gray-500">自由控制大小</div>
        </div>
      </div>
      ...
      {file && (
        <div className="space-y-6 p-6 bg-gray-50 rounded-lg">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <label className="font-medium text-gray-700">最大边长（像素）</label>
              <span className="text-blue-600 font-medium">{maxSize}px</span>
            </div>
            <input
              type="range"
              min="800"
        ...
        <div className="space-y-4 p-6 bg-green-50 rounded-lg">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <span className="font-medium">{(file.size / 1024).toFixed(0)} KB</span>
            </div>
            <div>
              <span className="text-gray-500">压缩后：</span>
              <span className="font-medium text-green-600">{(result.size / 1024).toFixed(0)} KB</span>
            </div>
          </div>
          <DownloadButton
            blob={result.blob}
